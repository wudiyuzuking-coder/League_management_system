...
public class MatchInfoServiceImpl implements MatchInfoService {
    ...
    @Transactional public MatchInfo updateStatus(Long id,String status){MatchInfo match=getById(id);if(!STATUSES.contains(status))throw new BusinessException("invalid match status");if(match.getMatchStatus().equals(status))return match;if(!NEXT.get(match.getMatchStatus()).contains(status))throw new BusinessException("invalid match status transition: "+match.getMatchStatus()+" -> "+status);if("FINISHED".equals(status)&&(match.getHomeScore()==null||match.getAwayScore()==null))throw new BusinessException("match score is required before finishing");if("PUBLISHED".equals(status))mapper.publish(id);else mapper.updateStatus(id,status);MatchInfo result=getById(id);if("FINISHED".equals(status))recalculateService.recalculate(match.getSeasonId());return result;}
    @Transactional public MatchInfo updateScore(Long id,MatchScoreRequest request){MatchInfo match=getById(id);if(!Set.of("IN_PROGRESS","FINISHED").contains(match.getMatchStatus()))throw new BusinessException("score can only be maintained for in-progress or finished matches");mapper.updateScore(id,request.homeScore(),request.awayScore());if("FINISHED".equals(match.getMatchStatus()))recalculateService.recalculate(match.getSeasonId());return getById(id);}
