...
class LeagueManagementIntegrationTest {
    ...
    @Test void roundValidationAuthorizationAndStateMachine() throws Exception {
        ...
        mockMvc.perform(post("/api/admin/seasons/{id}/rounds",seasonId).header("Authorization",bearer(eventAdminToken)).contentType(MediaType.APPLICATION_JSON)
    ...
    @Test void standingsAreInitializedCalculatedSortedAndProtected() throws Exception {
        ...
        updateRecord(ids.get(0),2,0,0,5,1,eventAdminToken).andExpect(status().isOk()).andExpect(jsonPath("$.data.points").value(6)).andExpect(jsonPath("$.data.matchesPlayed").value(2)).andExpect(jsonPath("$.data.goalDifference").value(4));
