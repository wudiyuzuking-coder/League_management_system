...
class SeatAllocationIntegrationTest {
    ...
    long matchZoneId;String admin,user,club,checker;
    ...
    @Test void previewRolesDebugPermissionValidationAndNoSideEffects()throws Exception{
        ...
        for(String token:List.of(user,club,checker,admin))preview(2,token).andExpect(status().isOk()).andExpect(jsonPath("$.data.seatNos[0]").value(2)).andExpect(jsonPath("$.data.seatNos[1]").value(3));
