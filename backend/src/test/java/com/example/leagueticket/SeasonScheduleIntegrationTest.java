...
class SeasonScheduleIntegrationTest {
    ...
    @Test void onlyEventAdminCanGenerateAndConfirm() throws Exception {
        ...
        for(String username:List.of("demo_user","demo_club","demo_admin"))mvc.perform(post("/api/admin/seasons/{id}/schedule/generate",season).header("Authorization",bearer(login(username)))).andExpect(status().isForbidden());
        mvc.perform(post("/api/admin/seasons/{id}/schedule/generate",season).header("Authorization",bearer(login("demo_event_admin")))).andExpect(status().isConflict());
    }
