...
四类正式角色统一使用账号资料页，可修改昵称、手机号和真实姓名；`EVENT_ADMIN`、`ADMIN`只读展示独立工号，`CLUB`只读展示绑定的`clubId`。资料页和顶部栏共用当前用户状态，资料更新、头像上传或移除后立即刷新，昵称和头像在页面重载后仍保持。

`sys_user.avatar_url`只保存相对访问路径，不保存图片二进制。开发和课程演示环境默认把文件写入项目外部可配置目录`APP_UPLOAD_DIR`（未设置时为`./uploads`），头像位于其`avatars`子目录并通过`/uploads/avatars/**`访问；运行时目录已加入Git忽略。上传接口为`POST /api/profile/avatar`，移除接口为`DELETE /api/profile/avatar`，读取图片无需登录，修改操作必须登录且始终作用于JWT中的当前`userId`。

仅允许JPEG、PNG且单文件最大2MB。后端同时校验扩展名、Content-Type和图片真实内容，拒绝空文件、伪造图片、格式不一致及超限文件；保存名使用UUID，不采用用户原文件名，并将路径限制在固定头像目录中。已有数据库执行`database/migrations/phase17c_user_avatar.sql`即可增加可空字段，历史账号默认继续使用系统默认头像。

> 本地文件系统方案仅用于开发和课程演示。生产环境应改用对象存储或受控文件服务，并配套访问域名、病毒扫描、生命周期和备份策略。

## USER俱乐部详情与比赛购票信息链（阶段18A）

USER可从比赛列表或比赛详情分别进入主队、客队的公开俱乐部详情。详情仅展示俱乐部与主场基础信息、ACTIVE球员、ACTIVE教练、当前或最近正式赛季积分、最近5场已结束公开比赛、未来5场已发布比赛，以及基于`SystemTimeService`计算的下一场比赛剩余天数；不返回账号、手机号、工号、报名快照、锁座信息或球员出生日期等私有/技术字段。

比赛详情补全场馆地址和容量，并按票区展示票价、销售时间、余票、最大连续座位数、物理座位数、有效座位数、排数与座号范围。是否可购买由后端结合比赛状态、销售窗口、统一系统时间和库存共同判断；连坐Preview与正式下单继续复用原有算法及交易链路，阶段18A没有修改数据库结构、座位分配排序、订单、支付或退票状态机。

主要USER接口：

- `GET /api/user/clubs/{clubId}`：安全的俱乐部公开聚合详情，仅允许USER访问。
- `GET /api/ticket-zones/match/{matchId}`、`GET /api/ticket-zones/{id}`：返回面向购票者的安全票区与座位结构摘要，不暴露内部锁座/售出明细。

## 阶段20A UX、阶段20B1负责人模型与阶段20B2自动开售

- Router与认证状态恢复统一使用`USER / CLUB / EVENT_ADMIN / ADMIN`白名单。缓存角色为空、`CHECKER`、未知字符串、Token缺失或服务端当前角色与缓存不一致时，会同时清除localStorage/sessionStorage中的Token和用户缓存并回到登录页，不会在登录页与角色首页之间循环。
- USER订单详情对`PAID`订单已有醒目的“申请整单退票”主入口，并继续复用原退款API和服务端最终业务校验，因此本阶段无需改退款业务。比赛列表、比赛详情的主客队链接以及`/user/clubs/:clubId`也已完成并保持可用。
- 阶段20B2起，票区`sale_start_time`固定派生为“比赛日期减7天的20:00”，客户端不能提交或修改。`ON_SALE`表示已启用自动销售，可以在自动开售前设置；真正可购还必须同时满足比赛`PUBLISHED`、统一系统时间位于`[sale_start_time, sale_end_time)`以及存在可用库存。查询、连坐Preview和正式下单共用同一`TicketSalePolicy`，系统时间调整后前端立即刷新。
- EVENT_ADMIN仍人工设置`sale_end_time`，结束边界保持排他，并且必须晚于自动开售时间且不超过原比赛截止规则。比赛改期时会锁定比赛与全部票区，原子重算所有票区开售时间；任一票区窗口冲突则整次改期返回409并回滚，既有已支付订单不会被改期撤销。
- CLUB负责人关系为`sys_user.club_id → club_info.club_id`的一对一绑定，数据库`UNIQUE(club_id)`作为最终并发防线，多个待审核账号仍可保持`club_id=NULL`。ADMIN审核会锁定账号和目标俱乐部，重复审核、并发绑定同一俱乐部以及绕过审核启用均返回友好业务错误。俱乐部列表、详情和CLUB资料页分别展示负责人姓名、手机号、昵称及状态；负责人被停用后绑定仍保留，俱乐部不会因此变为“无负责人”。
...
└─ 文档资料/
   ├─ 官方要求/           课程要求与样例文档
   ├─ 项目文档/           需求设计书、测试报告与历史文档
   └─ 更新与待修改事项/   更新日志和本次测试问题汇报
```
...
也可从PowerShell显式交给`cmd /c`执行，避免PowerShell把`<`当作自身语法：

```powershell
cmd /c "chcp 65001 > nul && mysql --default-character-set=utf8mb4 -u root -p league_ticket < database\schema.sql"
```
...
旧库不得通过回退当前代码或删表来“兼容”。先完整备份并确认它确实是阶段16B以前的可识别25表版本，再按顺序、每个脚本仅执行一次：

1. `database/migrations/phase16b_club_season_enrollment.sql`
2. `database/migrations/phase16c_auto_schedule.sql`
3. `database/migrations/phase17a_employee_no.sql`
4. `database/migrations/phase17b_phone_login.sql`
5. `database/migrations/phase17c_user_avatar.sql`

迁移前必须检查备份可恢复、当前表/列/索引版本、空或重复手机号以及历史管理账号工号；若无法确认旧库版本，不要自动执行，应先与当前`schema.sql`做结构差异审计。阶段19已从可靠旧基线实际验证该链可升级为30表且与当前schema的列、索引和约束一致。

`scripts/env.ps1`不再写死个人MySQL目录、固定3315端口、数据库口令或JWT密钥。运行后端前由调用者设置`DB_URL`、`DB_USERNAME`、`DB_PASSWORD`和`JWT_SECRET`；MySQL命令行可通过`MYSQL_EXE`指定，使用本地启动辅助脚本时另设`MYSQL_HOST`、`MYSQL_PORT`和`MYSQL_DATA_DIR`。
...
生产JAR启动前先按下文设置`SPRING_PROFILES_ACTIVE=dev`、数据库、JWT和上传目录环境变量，然后执行：

```powershell
cd backend
.\mvnw.cmd clean package
java -jar target\league-ticket-backend-0.0.1-SNAPSHOT.jar
```

阶段19已用上述可执行JAR连接全新隔离MySQL 8实例实测启动成功，`/api/health`、手机号登录和核心业务接口均正常；最终运行不依赖IDE临时classpath。
...
登录成功返回仅有一个Access Token，不实现Refresh Token。Token默认有效期2小时，`sub`为稳定的`userId`，并可携带`username`、`phone`和`roleCode`快照；后端授权会按userId重新加载账号。前端通过 `Authorization: Bearer <token>` 发送。阶段17B上线前签发的旧Token需要重新登录。

完成上述密码初始化后，可使用统一演示密码 `123456`：

| 角色 | 登录手机号 | 昵称 | 姓名/名称 | 工号 | 前端入口 |
|---|---|---|---|---|---|
| USER | `13800000001` | `demo_user` | 演示普通用户 | — | `/user` |
| CLUB | `13800000003` | `demo_club` | 潮汐俱乐部负责人 | — | `/club` |
| EVENT_ADMIN | `13800000005` | `demo_event_admin` | 演示赛事管理员 | `EA0001` | `/admin` |
| ADMIN | `13800000002` | `demo_admin` | 演示系统管理员 | `SA0001` | `/admin` |
...
- [完整测试报告](../文档资料/项目文档/docs/test-report/TEST_REPORT.md)
- [大模型辅助开发使用说明](../文档资料/项目文档/docs/ai-usage/AI_USAGE.md)
- [需求文档索引](../文档资料/项目文档/docs/requirements/README.md)
- [设计文档索引](../文档资料/项目文档/docs/design/README.md)

当前四角色版本使用全新隔离 MySQL 8.0.44 完成阶段19最终验收：共107项测试，`Failures=0`、`Errors=0`、`Skipped=0`；`clean test`、不跳过测试的 `clean package`、可执行JAR真实启动和前端生产构建均成功。空库初始化及数据脚本二次执行保持30表、4角色、4演示账号；四角色真实HTTP权限、核心E2E、15项数据一致性和浏览器主流程均已复核。完整证据见[阶段19最终审计](../文档资料/项目文档/docs/PHASE19_FINAL_AUDIT.md)和[测试报告](../文档资料/项目文档/docs/test-report/TEST_REPORT.md)。
...
- 独立CHECKER和检票是历史模块，代码与旧测试保留用于追溯，但不在正式角色、菜单、演示账号和本次验收范围内。
- 场馆当前采用通用`stadium/stadium_zone/stadium_seat`模型，未强制“东/西/南/北 × VIP/普通”固定八区模板，也未增加长边/宽边每排座位数字段；通用模型已满足当前场馆、座位和购票演示。
- 主场未保存VIP/普通默认价格；当前正式价格按比赛票区存于`match_ticket_zone.ticket_price`。
- `coach_info`没有出生日期或出生年，因此教练年龄未实现；报名仍要求至少1名有效教练。
- 阶段20A已统一管理账号入口：EVENT_ADMIN和ADMIN只能由ADMIN后台创建；此前公开申请形成的历史管理账号不删除，仍可在资料完整并启用后使用。
- 短信验证码、找回密码、OAuth、云对象存储、点球/加时、AI排赛/推荐、评论、收藏、手工点座和球员转会均不属于当前正式范围。

上述项目不影响当前正式验收范围，也不会为了让旧待办表面清零而在冻结阶段强行实现。
