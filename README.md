...
- 管理系统用户、角色归属和账号状态。
- 创建、编辑和启停俱乐部，维护俱乐部球员、教练及赛季数据。
- 审核 CLUB 注册申请，可创建新俱乐部或绑定尚无负责人的已有俱乐部。
- 维护本人账号资料。
...
`Program file/database/schema.sql` 创建 `league_ticket` 数据库及 30 张表。主要数据域包括：
...
前端当前没有配置通用 `npm test` 脚本。认证缓存测试可直接运行：

```powershell
cd "Program file\frontend"
node --test tests/authStorage.test.js
```
...
- 联赛管理、俱乐部报名与赛程、场馆座位、比赛票务、订单锁座、模拟支付、电子票、退票审核和运营统计均有对应前端页面与后端实现。
...
- `mvnw clean test`：成功；发现 130 项测试，执行 11 项、跳过 119 项。跳过项为未启用 `RUN_DB_TESTS` 的数据库集成测试。
