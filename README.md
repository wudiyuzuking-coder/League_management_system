...
- `CLUB`：维护账号绑定俱乐部的资料、球员、教练和球员赛季数据，查看本队比赛、主场票务和统计。
- `CHECKER`：查看本俱乐部主场比赛，核验电子票并查看本人检票记录。
- `ADMIN`：管理用户、俱乐部、赛季、轮次、比赛、场馆、静态票区、物理座位和比赛票务，审核退票并查看检票与统计数据。

系统不采用前端菜单隐藏作为权限保障。JWT认证、角色权限和CLUB/CHECKER的俱乐部数据范围均由后端校验。
