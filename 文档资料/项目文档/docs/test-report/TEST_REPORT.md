...
## 阶段20B2 自动开售验收（2026-09-03）

- 完整命令：`RUN_DB_TESTS=true mvnw clean test`
- 结果：`Tests run: 130, Failures: 0, Errors: 0, Skipped: 0`，`BUILD SUCCESS`
- 原阶段20B1的123项测试全部保留；新增7项覆盖自动开售公式、客户端不可覆盖、19:59/20:00边界、DRAFT/PAUSED/CLOSED/零库存、排他结束边界、比赛改期多票区重算与冲突回滚、已支付订单保留及系统时间立即生效。
- 查询、连坐Preview、下单统一复用`TicketSalePolicy`；`ON_SALE`只表示运营启用，真正可购还需满足比赛状态、自动时间窗和库存条件。
- 数据库仍为30表且无新增字段；冷库初始化后票区派生时间一致，`phase20b2_auto_sale_start.sql`在隔离库连续执行两次均通过冲突前置检查并保持幂等。
...
- A（开售前）：系统时间设为`2026-08-29 19:59`，票区查询为`NOT_STARTED + saleAvailable=false`，连坐Preview和下单均返回409。
- B（开售边界）：系统时间设为`2026-08-29 20:00`，查询、Preview和下单均为200；模拟支付后为`order=PAID`、`item=PAID`、`ticket=UNUSED`。
- C（延后比赛）：EVENT_ADMIN将比赛从9月5日延至9月6日，更新返回200，全部票区开售时间统一变为`2026-08-30 20:00`；B中已支付订单仍为PAID、明细PAID、电子票UNUSED。
- D（提前比赛）：比赛改回9月5日后，全部派生时间恢复为`2026-08-29 20:00`，在当前系统时间立即可售。
- E（停售边界）：系统时间精确到`sale_end_time`时为`ENDED + saleAvailable=false`，继续下单返回409。
- 浏览器确认USER比赛详情在19:58显示“未开售”且Preview/购票按钮禁用；同页调整系统时间到20:00后无需重登或手工刷新即变为“销售中”并启用按钮。EVENT_ADMIN票务页分列显示`ON_SALE / 已启用自动销售`、当前销售状态、自动开售时间和停售时间，在开售前明确显示“尚未开售”；新增表单只读展示派生时间且请求不再提供可编辑开售字段。页面无新增控制台错误。
- 本轮JAR和HTTP仅连接`127.0.0.1:3317/league_ticket_phase20b2`隔离库。由于未提供正式开发库连接，本轮未执行正式库迁移前置审计；迁移脚本会先报告冲突并只在冲突数为0时统一回填。
...
- `clean test`：BUILD SUCCESS，130/0/0/0。
- `clean package`：未使用 `-DskipTests`，BUILD SUCCESS，130/0/0/0，并生成可执行JAR。
