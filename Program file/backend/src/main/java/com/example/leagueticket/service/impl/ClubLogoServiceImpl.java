...
public class ClubLogoServiceImpl implements ClubLogoService {
    ...
    @Override @Transactional public AvatarResponse upload(Long clubId,MultipartFile file){
        ...
        String contentType=Optional.ofNullable(file.getContentType()).orElse("").toLowerCase(Locale.ROOT).split(";",2)[0].trim();
        if(!Set.of("image/jpeg","image/png","application/octet-stream","").contains(contentType))throw new BusinessException("队徽仅支持JPEG或PNG格式");
        ...
        String ext=detect(file,contentType);if((ext.equals(".png")&&!original.endsWith(".png"))||(ext.equals(".jpg")&&!(original.endsWith(".jpg")||original.endsWith(".jpeg"))))throw new BusinessException("队徽文件扩展名与实际内容不一致");
    ...
    private String detect(MultipartFile file,String contentType){try(InputStream in=file.getInputStream();ImageInputStream image=ImageIO.createImageInputStream(in)){if(image==null)throw new BusinessException("队徽文件不是有效图片");Iterator<ImageReader> readers=ImageIO.getImageReaders(image);if(!readers.hasNext())throw new BusinessException("队徽文件不是有效图片");ImageReader reader=readers.next();try{String ext=EXT.get(reader.getFormatName().toUpperCase(Locale.ROOT));reader.setInput(image,true,true);if(ext==null||reader.getWidth(0)<=0||reader.getHeight(0)<=0)throw new BusinessException("队徽仅支持JPEG或PNG格式");if(("image/png".equals(contentType)&&!ext.equals(".png"))||("image/jpeg".equals(contentType)&&!ext.equals(".jpg")))throw new BusinessException("队徽文件类型与实际内容不一致");return ext;}finally{reader.dispose();}}catch(IOException e){throw new BusinessException("队徽文件不是有效图片");}}
