...
public class ClubSeasonRecordServiceImpl implements ClubSeasonRecordService {
    ...
    @Transactional public int initialize(Long seasonId){SeasonInfo season=seasonService.getById(seasonId);requireSeasonWritable(season);return mapper.initializeActiveClubs(seasonId);}
    @Transactional public StandingResponse update(Long recordId,SeasonRecordRequest request){ClubSeasonRecord record=mapper.findById(recordId);if(record==null)throw new BusinessException(HttpStatus.NOT_FOUND,"club season record not found");requireSeasonWritable(seasonService.getById(record.getSeasonId()));record.setWins(request.wins());record.setDraws(request.draws());record.setLosses(request.losses());record.setGoalsFor(request.goalsFor());record.setGoalsAgainst(request.goalsAgainst());record.setPlayed(request.wins()+request.draws()+request.losses());record.setPoints(request.wins()*3+request.draws());mapper.update(record);return standings(record.getSeasonId()).stream().filter(v->v.recordId().equals(recordId)).findFirst().orElseThrow();}
    private void requireSeasonWritable(SeasonInfo season){if(SeasonStatus.CANCELLED.matches(season.getSeasonStatus()))throw new BusinessException(HttpStatus.CONFLICT,"已取消赛季仅允许查看，不能修改积分数据");}
