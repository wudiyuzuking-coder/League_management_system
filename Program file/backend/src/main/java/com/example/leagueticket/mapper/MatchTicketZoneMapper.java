...
public interface MatchTicketZoneMapper {
    String SELECT_COLUMNS="mz.match_zone_id,mz.match_id,mz.stadium_zone_id,mz.created_by,mz.zone_name_snapshot,sz.zone_code,sz.zone_direction,sz.ticket_type,mz.ticket_price,mz.zone_status,mz.sale_start_time,mz.sale_end_time,mz.version,mz.created_at,mz.updated_at";
    ...
    @Update("UPDATE match_ticket_zone SET sale_start_time=#{saleStartTime},sale_end_time=#{saleEndTime},version=version+1 WHERE match_id=#{matchId}")
    int updateSaleWindowByMatch(@Param("matchId")Long matchId,@Param("saleStartTime")java.time.LocalDateTime saleStartTime,@Param("saleEndTime")java.time.LocalDateTime saleEndTime);
}
