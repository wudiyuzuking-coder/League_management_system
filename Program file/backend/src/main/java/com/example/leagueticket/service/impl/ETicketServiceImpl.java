...
public class ETicketServiceImpl implements ETicketService {
    ...
    public PageResponse<ETicketResponse> listOwned(Long userId, TicketQueryRequest query) {
        ...
        List<ETicketResponse> records = mapper
                .findOwnedPage(userId, status, (long) (query.getPage() - 1) * query.getSize(), query.getSize()).stream()
                .map(ETicketServiceImpl::response).toList();
    ...
    public ETicketResponse detailOwned(Long userId, Long ticketId) {
        ...
        if (ticket == null)
            throw new BusinessException(HttpStatus.NOT_FOUND, "ticket not found");
    ...
    private String normalize(String status) {
        ...
        if (!STATUSES.contains(value))
            throw new BusinessException("invalid ticketStatus");
        return value;
    }

    static ETicketResponse response(ETicket t) {
        return new ETicketResponse(t.getTicketId(), t.getTicketCode(), t.getOrderId(), t.getItemId(),
                t.getTicketStatus(), t.getIssuedAt(), t.getUsedAt(), t.getMatchId(), t.getHomeClubName(),
                t.getAwayClubName(), t.getMatchTime(), t.getStadiumName(), t.getZoneName(), t.getRowNo(), t.getSeatNo(),
                t.getPassengerName(), t.getPassengerIdCard(), t.getTicketPrice());
    }
}
