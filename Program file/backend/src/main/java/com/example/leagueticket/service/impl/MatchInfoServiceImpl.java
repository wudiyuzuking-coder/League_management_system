...
public class MatchInfoServiceImpl implements MatchInfoService {
    ...
    @Transactional public MatchInfo create(MatchRequest request){validateBusiness(request,null);MatchInfo match=copy(new MatchInfo(),request);ticketSalePolicy.applySaleWindow(match);ticketSalePolicy.validateSaleWindow(match);mapper.insert(match);return getById(match.getMatchId());}
