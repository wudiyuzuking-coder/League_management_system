...
public class MatchInfoServiceImpl implements MatchInfoService {
    ...
    public PageResponse<MatchInfo> list(MatchQueryRequest query){validateQuery(query);long total=mapper.count(query);long offset=(long)(query.getPage()-1)*query.getSize();var rows=mapper.findPage(query,offset,query.getSize());rows.forEach(ticketSalePolicy::applySaleWindow);return new PageResponse<>(rows,total,query.getPage(),query.getSize());}
    public PageResponse<MatchInfo> listPublic(MatchQueryRequest query){query.setPublicOnly(true);return list(query);}
    public MatchInfo getById(Long id){MatchInfo value=mapper.findById(id);if(value==null)throw new BusinessException(HttpStatus.NOT_FOUND,"match not found");ticketSalePolicy.applySaleWindow(value);return value;}
    ...
    @Transactional public MatchInfo update(Long id,MatchRequest request){MatchInfo current=mapper.findByIdForUpdate(id);if(current==null)throw new BusinessException(HttpStatus.NOT_FOUND,"match not found");if(Set.of("IN_PROGRESS","FINISHED","CANCELLED").contains(current.getMatchStatus()))throw new BusinessException("match basic information cannot be changed in current status");if("PUBLISHED".equals(current.getMatchStatus())&&!sameExceptTime(current,request))throw new BusinessException("published match only allows match time changes");validateBusiness(request,id);ticketZoneMapper.findByMatchForUpdate(id);MatchInfo changed=copy(current,request);ticketSalePolicy.applySaleWindow(changed);ticketSalePolicy.validateSaleWindow(changed);if("PUBLISHED".equals(current.getMatchStatus()))mapper.updateTimeAndSale(changed);else mapper.updateBasic(changed);ticketZoneMapper.updateSaleWindowByMatch(id,changed.getSaleStartTime(),changed.getSaleEndTime());return getById(id);}
    @Transactional public MatchInfo updateStatus(Long id,String status){MatchInfo match=getById(id);if(!STATUSES.contains(status))throw new BusinessException("invalid match status");if(Set.of("PUBLISHED","IN_PROGRESS","CANCELLED").contains(status))throw new BusinessException(HttpStatus.CONFLICT,"比赛发布、开始和取消均由系统自动维护");if("FINISHED".equals(status))throw new BusinessException(HttpStatus.CONFLICT,"比赛只能通过多人赛果确认流程结束");if(match.getMatchStatus().equals(status))return match;throw new BusinessException("invalid match status transition: "+match.getMatchStatus()+" -> "+status);}
    @Transactional public int startPublishedMatchesDue(){return mapper.startPublishedMatchesDue(systemTimeService.now());}
    @Transactional public boolean startPublishedMatchIfDue(Long id){return mapper.startPublishedMatchIfDue(id,systemTimeService.now())==1;}
