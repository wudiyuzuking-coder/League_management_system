...
public class StadiumInfoServiceImpl implements StadiumInfoService {
    ...
    private StadiumInfo copy(StadiumInfo s,StadiumRequest r){s.setStadiumName(r.stadiumName().trim());s.setCity(r.city().trim());s.setAddress(r.address().trim());s.setCapacity(r.capacity());s.setLayoutDesc(r.layoutDesc()==null||r.layoutDesc().isBlank()?null:r.layoutDesc().trim());if(s.getVenueModel()==null)s.setVenueModel("LEGACY");return s;}
}
