...
public class RoundInfoServiceImpl implements RoundInfoService {
    ...
    @Transactional public RoundInfo create(Long seasonId,RoundRequest request){SeasonInfo season=seasonService.getById(seasonId);requireSeasonWritable(season);validate(season,request,null);RoundInfo round=copy(new RoundInfo(),request);round.setSeasonId(seasonId);round.setRoundStatus("DRAFT");mapper.insert(round);return getById(round.getRoundId());}
    @Transactional public RoundInfo update(Long id,RoundRequest request){RoundInfo round=getById(id);SeasonInfo season=seasonService.getById(round.getSeasonId());requireSeasonWritable(season);validate(season,request,id);mapper.update(copy(round,request));return getById(id);}
    @Transactional public RoundInfo updateStatus(Long id,String status){RoundInfo round=getById(id);requireSeasonWritable(seasonService.getById(round.getSeasonId()));if(round.getRoundStatus().equals(status))return round;String next=NEXT.get(round.getRoundStatus());if(!status.equals(next))throw new BusinessException("invalid round status transition: "+round.getRoundStatus()+" -> "+status);mapper.updateStatus(id,status);return getById(id);}
    private void requireSeasonWritable(SeasonInfo season){if(SeasonStatus.CANCELLED.matches(season.getSeasonStatus()))throw new BusinessException(HttpStatus.CONFLICT,"已取消赛季仅允许查看，不能修改轮次");}
