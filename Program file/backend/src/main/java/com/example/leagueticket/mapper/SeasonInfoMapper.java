...
public interface SeasonInfoMapper {
  ...
  int insert(SeasonInfo season);

  @Update("UPDATE season_info SET season_name=#{seasonName},start_date=#{startDate},end_date=#{endDate},registration_start_time=#{registrationStartTime},registration_deadline=#{registrationDeadline},ticket_sale_start_time=#{ticketSaleStartTime},max_clubs=#{maxClubs} WHERE season_id=#{seasonId}")
  int update(SeasonInfo season);
