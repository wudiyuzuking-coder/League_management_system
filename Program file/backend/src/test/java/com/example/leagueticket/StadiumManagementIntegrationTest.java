...
class StadiumManagementIntegrationTest {
    ...
    @Test void stadiumCrudValidationAndPermission()throws Exception{
        ...
        createStadiumRequest("IT7中心场",100,systemAdmin).andExpect(status().isConflict());
        createStadiumRequest("IT7零容量",0,systemAdmin).andExpect(status().isBadRequest());
        createStadiumRequest("IT7用户越权",100,user).andExpect(status().isForbidden());
        createStadiumRequest("IT7赛事管理员越权",100,admin).andExpect(status().isForbidden());
        mockMvc.perform(put("/api/admin/stadiums/{id}",id).header("Authorization",bearer(systemAdmin)).contentType(MediaType.APPLICATION_JSON).content(json(stadium("IT7中心场更新",120)))).andExpect(status().isOk()).andExpect(jsonPath("$.data.capacity").value(120));
        mockMvc.perform(put("/api/admin/stadiums/{id}/status",id).header("Authorization",bearer(systemAdmin)).contentType(MediaType.APPLICATION_JSON).content("{\"stadiumStatus\":\"DISABLED\"}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.stadiumStatus").value("DISABLED"));
    ...
    @Test void zoneUniquenessExistenceAndPermission()throws Exception{
        ...
        createZoneRequest(stadiumId,"A","B区",2,systemAdmin).andExpect(status().isConflict());
        createZoneRequest(stadiumId,"B","A区",2,systemAdmin).andExpect(status().isConflict());
        createZoneRequest(999999L,"X","不存在",0,systemAdmin).andExpect(status().isNotFound());
        createZoneRequest(stadiumId,"B","B区",2,user).andExpect(status().isForbidden());
        createZoneRequest(stadiumId,"B","B区",2,admin).andExpect(status().isForbidden());
        mockMvc.perform(put("/api/admin/stadium-zones/{id}",zoneId).header("Authorization",bearer(systemAdmin)).contentType(MediaType.APPLICATION_JSON).content(json(zone("A2","A区更新",3)))).andExpect(status().isOk()).andExpect(jsonPath("$.data.sortNo").value(3));
    }
    ...
    @Test void seatsBatchLayoutStatusRollbackAndCapacity()throws Exception{
        ...
        seatRequest(zoneId,0,"0排",1,"1",systemAdmin).andExpect(status().isBadRequest());seatRequest(zoneId,1,"1排",0,"0",systemAdmin).andExpect(status().isBadRequest());
        mockMvc.perform(post("/api/admin/stadium-zones/{id}/seats/batch",zoneId).header("Authorization",bearer(systemAdmin)).contentType(MediaType.APPLICATION_JSON).content(batch())).andExpect(status().isOk()).andExpect(jsonPath("$.data").value(5));
        long before=jdbc.queryForObject("SELECT COUNT(*) FROM stadium_seat WHERE stadium_zone_id=?",Long.class,zoneId);
        mockMvc.perform(post("/api/admin/stadium-zones/{id}/seats/batch",zoneId).header("Authorization",bearer(systemAdmin)).contentType(MediaType.APPLICATION_JSON).content(batch())).andExpect(status().isConflict());
        ...
        seatRequest(zoneId,1,"1排",1,"1",systemAdmin).andExpect(status().isConflict());
        ...
        mockMvc.perform(put("/api/admin/stadium-seats/{id}/status",seatId).header("Authorization",bearer(systemAdmin)).contentType(MediaType.APPLICATION_JSON).content("{\"seatStatus\":\"DISABLED\"}")).andExpect(status().isOk());
        mockMvc.perform(get("/api/admin/stadiums/{id}/capacity-summary",stadiumId).header("Authorization",bearer(systemAdmin))).andExpect(status().isOk()).andExpect(jsonPath("$.data.declaredCapacity").value(500)).andExpect(jsonPath("$.data.totalSeatCount").value(5)).andExpect(jsonPath("$.data.activeSeatCount").value(4)).andExpect(jsonPath("$.data.disabledSeatCount").value(1));
    }

    private long createStadium(String name,int capacity)throws Exception{String body=createStadiumRequest(name,capacity,systemAdmin).andExpect(status().isOk()).andReturn().getResponse().getContentAsString();return objectMapper.readTree(body).path("data").path("stadiumId").asLong();}
    ...
    private long createZone(long stadiumId,String code,String name,int sort)throws Exception{String body=createZoneRequest(stadiumId,code,name,sort,systemAdmin).andExpect(status().isOk()).andReturn().getResponse().getContentAsString();return objectMapper.readTree(body).path("data").path("stadiumZoneId").asLong();}
