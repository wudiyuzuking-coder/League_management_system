...
<template><el-tag class="status-tag" :class="{ 'status-tag--live': isLive }" :type="type || statusType(value)" effect="light"
    :data-status="value"><span v-if="showDot" class="status-tag__dot"
      aria-hidden="true" />{{ label || statusLabel(value) }}</el-tag></template>
