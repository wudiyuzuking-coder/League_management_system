...
import { isRegistrationFormValid, PHONE_PATTERN } from '../../utils/authForm'
...
const canSubmit = computed(() => isRegistrationFormValid(form))
...
const rules = {
  ...
  realName: [
    ...
    { max: 80, message: '姓名不能超过80个字符', trigger: 'blur' },
  ],
  clubName: [
    { required: true, message: '请输入俱乐部名字', trigger: 'blur' },
    { max: 100, message: '俱乐部名称不能超过100个字符', trigger: 'blur' },
  ],
  phone: [
    { required: true, message: '请输入手机号', trigger: 'blur' },
    { pattern: PHONE_PATTERN, message: '请输入11位手机号', trigger: 'blur' },
  ],
  password: [
