...
const rules = {
  ...
  employeeNo: [{ validator: (_rule, value, done) => {
    ...
    const pattern = employeeNoPattern(form.roleCode)
...
const submit = async () => {
  ...
  try {
    ...
    if (!error?.__notified) ElMessage.error(getLoginErrorMessage(error))
  } finally {
    loading.value = false
  }
}
  ...
  <main class="auth-page">
    <el-card class="auth-card" shadow="hover">
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-position="top" @keyup.enter="submit">
        ...
        <el-button class="auth-submit" type="primary" :disabled="!canSubmit || loading" :loading="loading" @click="submit">登录</el-button>
      </el-form>
