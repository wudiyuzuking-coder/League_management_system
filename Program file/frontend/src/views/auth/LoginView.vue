...
<template><main class="auth-page"><el-card class="auth-card" shadow="hover"><template #header><div class="auth-heading"><span class="auth-kicker">LEAGUE TICKET</span><h1>足球联赛购票系统</h1><p>{{managementRole?'先验证手机号与 4 位工号数字':'选择身份后使用手机号登录'}}</p></div></template><el-form label-position="top" @keyup.enter="submit"><el-form-item label="身份"><el-select v-model="form.roleCode" name="role" placeholder="请选择身份…" style="width:100%"><el-option v-for="role in roleOptions" :key="role.code" :label="role.label" :value="role.code"/></el-select></el-form-item><el-form-item label="手机号"><el-input v-model="form.phone" name="phone" type="tel" inputmode="tel" autocomplete="tel" placeholder="例如：13800000000…"/></el-form-item><el-form-item v-if="managementRole" label="4 位工号数字"><el-input v-model="form.employeeNo" name="employee-no" inputmode="numeric" autocomplete="off" maxlength="4" placeholder="例如：0001…"/></el-form-item><template v-if="!managementRole||stage!=='IDENTITY'"><el-alert v-if="stage==='ACTIVATE'" title="首次启用：姓名必须与预登记信息完全一致" type="info" :closable="false"/><el-form-item v-if="stage==='ACTIVATE'" label="姓名"><el-input v-model="form.realName" name="real-name" autocomplete="name"/></el-form-item><el-form-item label="密码"><el-input v-model="form.password" name="password" type="password" show-password autocomplete="current-password"/></el-form-item><el-form-item v-if="stage==='ACTIVATE'" label="确认密码"><el-input v-model="form.confirmPassword" name="confirm-password" type="password" show-password autocomplete="new-password"/></el-form-item></template><el-button class="auth-submit" type="primary" :disabled="managementRole?(!identityValid||loading):(!canSubmit||loading)" :loading="loading" @click="submit">{{managementRole&&stage==='IDENTITY'?'下一步':stage==='ACTIVATE'?'启用并登录':'登录'}}</el-button></el-form><p class="auth-switch">还没有普通用户账号？<RouterLink to="/register">立即注册</RouterLink></p></el-card></main></template>
