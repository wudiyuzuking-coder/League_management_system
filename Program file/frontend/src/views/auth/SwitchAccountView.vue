...
<template><main class="auth-page"><el-card class="auth-card" shadow="hover"><template #header><div class="auth-heading"><span class="auth-kicker">SWITCH ACCOUNT</span><h1>切换账户</h1><p>验证新账户成功前，当前账户保持登录</p></div></template><el-alert :title="`当前账户：${authStore.user?.username} · ${maskPhone(authStore.user?.phone)} · ${ROLE_LABELS[authStore.user?.roleCode]}`" type="info" :closable="false"/><el-form ref="formRef" :model="form" :rules="rules" label-position="top"><el-form-item label="新账户身份" prop="roleCode"><el-select v-model="form.roleCode" style="width:100%"><el-option v-for="(label,code) in ROLE_LABELS" :key="code" :label="label" :value="code"/></el-select></el-form-item><el-form-item label="手机号" prop="phone"><el-input v-model="form.phone"/></el-form-item><el-form-item v-if="MANAGEMENT_ROLES.includes(form.roleCode)" label="4位工号数字" prop="employeeNo"><el-input v-model="form.employeeNo" maxlength="4" placeholder="例如：0001"/></el-form-item><el-form-item label="密码" prop="password"><el-input v-model="form.password" type="password" show-password/></el-form-item><el-button class="auth-submit" type="primary" :disabled="!canSubmit" :loading="loading" @click="submit">确认切换</el-button><el-button @click="router.back()">返回当前账户</el-button></el-form></el-card></main></template>
