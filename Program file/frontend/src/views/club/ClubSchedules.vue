...
import {getClubSchedules,getSeasonNotifications} from '../../api/club'

const rows=ref([]),notifications=ref([]),loading=ref(false),error=ref(''),expanded=ref([])
...
const loadNotifications=async()=>{try{notifications.value=(await getSeasonNotifications()).data||[]}catch(e){console.warn('加载赛季取消通知失败',e);notifications.value=[]}}
const load=async()=>{loading.value=true;error.value='';void loadNotifications();try{rows.value=(await getClubSchedules()).data}catch(e){error.value=e?.message||'加载俱乐部赛程失败，请稍后重试。'}finally{loading.value=false}}
...
<template>
  <div>
    ...
    <section v-if="notifications.length" class="cancellation-notices" aria-label="已取消赛季提示">
      <CardShell v-for="notice in notifications" :key="notice.notificationId" variant="action" compact>
        <div class="cancellation-notice"><div><strong>{{notice.seasonName}}</strong><p>{{notice.message||'因参赛俱乐部不足 2 支，该赛季已取消'}}</p><small v-if="notice.cancelledAt">取消时间：{{$formatDateTime(notice.cancelledAt)}}</small></div><StatusTag value="CANCELLED" /></div>
      </CardShell>
    </section>
...
.cancellation-notices{display:grid;gap:var(--space-3);margin:var(--space-4) 0}.cancellation-notice{display:flex;align-items:flex-start;justify-content:space-between;gap:var(--space-4)}.cancellation-notice strong{font-size:var(--font-size-lg)}.cancellation-notice p{margin:var(--space-1) 0;color:var(--color-text-secondary)}.cancellation-notice small{color:var(--color-text-muted)}
</style>
