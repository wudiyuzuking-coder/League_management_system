...
const router=useRouter(),loading=ref(false),overview=ref({players:[],coaches:[]})
...
const load=async()=>{loading.value=true;try{overview.value=(await getClubPersonnelOverview()).data}finally{loading.value=false}}
onMounted(load)
</script>
...
<template>
  <div v-loading="loading">
    <el-card>
      <template #header><div class="head"><h2>人员管理</h2><div><el-button @click="router.push('/club/players')">球员管理</el-button><el-button @click="router.push('/club/coaches')">教练管理</el-button></div></div></template>
