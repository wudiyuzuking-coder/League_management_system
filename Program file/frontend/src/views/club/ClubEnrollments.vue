...
const cancellationMessage=row=>row?.cancelReason?`因${row.cancelReason}，该赛季已取消`:'该赛季已取消'
...
<template>
  <div>
    ...
    <el-dialog v-model="detailVisible" class="app-dialog" title="报名阵容快照" width="850px"><DataState :loading="detailLoading" :error="detailError" :empty="false" @retry="show(detailId)"><MetricStrip :items="[{label:'赛季',value:detail.seasonName},{label:'报名状态',status:detail.enrollmentStatus},{label:'赛季状态',status:detail.seasonStatus},{label:'主场',value:detail.stadiumName},{label:'提交时间',value:formatDateTime(detail.submittedAt)}]" label="报名快照摘要"/><el-alert v-if="detail.seasonStatus==='CANCELLED'" class="dialog-section" type="error" :closable="false" :title="cancellationMessage(detail)"><span v-if="detail.cancelledAt">取消时间：{{formatDateTime(detail.cancelledAt)}}</span></el-alert><TableWrapper class="dialog-section" title="报名球员" label="报名球员"><el-table :data="detail.players||[]"><el-table-column prop="shirtNo" label="号码" width="70"/><el-table-column prop="playerName" label="姓名"/><el-table-column label="位置"><template #default="{row}">{{positionLabel[row.position]}}</template></el-table-column><el-table-column label="阵容"><template #default="{row}"><StatusTag :value="row.lineupRole"/></template></el-table-column></el-table></TableWrapper><TableWrapper class="dialog-section" title="报名教练" label="报名教练"><el-table :data="detail.coaches||[]"><el-table-column prop="coachName" label="姓名"/><el-table-column label="职务"><template #default="{row}">{{titleLabel[row.title]}}</template></el-table-column></el-table></TableWrapper></DataState></el-dialog>
  </div>
</template>
...
.season-status{display:grid;justify-items:start;gap:var(--space-1)}.season-status span,.season-status small{color:var(--color-text-muted);font-size:var(--font-size-xs)}
</style>
