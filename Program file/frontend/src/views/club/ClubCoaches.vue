...
const coachAgeError=computed(()=>{
  if(form.birthYear===null||form.birthYear==='')return ''
  const year=Number(form.birthYear),age=currentYear.value-year
  return Number.isInteger(year)&&age>=18&&age<=100?'':'教练年龄必须在18至100岁之间'
})
const birthYearValidator=(_r,_value,done)=>coachAgeError.value?done(new Error(coachAgeError.value)):done()
const rules={coachName:[{required:true,message:'请输入教练姓名'}],nationality:[{required:true,message:'请输入国籍'}],birthYear:[{required:true,message:'请输入出生年份'},{validator:birthYearValidator,trigger:['change','blur']}],title:[{required:true,message:'请选择职务'}]}
...
const save=async()=>{if(coachAgeError.value)return;await formRef.value.validate();saving.value=true;try{editingId.value?await updateClubCoach(editingId.value,{title:form.title}):await createClubCoach({...form,birthYear:Number(form.birthYear)});visible.value=false;ElMessage.success('保存成功');await load()}finally{saving.value=false}}
    ...
    <DataState :loading="loading" :error="error" :empty="!rows.length" empty-title="暂无教练" empty-description="新增教练后即可组建教练团队。" @retry="load">
      <TableWrapper title="教练组" description="主教练优先展示，离队人员保留为可追溯记录。" label="教练列表">
        <el-table :data="displayed" row-key="coachId">
          ...
          <el-table-column label="年龄" width="90"><template #default="{row}">{{row.age??(row.birthYear?currentYear-row.birthYear:'—')}}</template></el-table-column>
