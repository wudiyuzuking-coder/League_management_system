    ...
    <DataState :loading="loading" :error="error" :empty="!match.matchId" empty-title="比赛不存在或暂不可查看" @retry="load">
      <section class="match-hero">
        ...
        <div class="match-hero__fixture">
          ...
          <div class="match-score tabular-nums"><strong>{{match.homeScore==null?'VS':`${match.homeScore} : ${match.awayScore}`}}</strong><small>{{match.homeScore==null?'MATCH DAY':'全场比分'}}</small></div>
        ...
        <div class="match-hero__details">
          ...
          <span><small>剩余票数</small><b class="tabular-nums">{{remaining}}</b></span>
        </div>
      </section>
      <TicketZoneList :match-id="route.params.id" />
    </DataState>
  </div>
</template>
...
.match-hero__details{display:grid;grid-template-columns:1.1fr 1fr 1.5fr .7fr;gap:var(--space-md);padding:18px 24px;border-top:1px solid var(--border-color);background:#fbfcfb}.match-hero__details span{display:flex;flex-direction:column;gap:5px}.match-hero__details small{color:var(--text-muted)}.match-hero__details b{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
</style>
