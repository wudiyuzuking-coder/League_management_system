...
const route=useRoute(),router=useRouter(),systemTime=useSystemTimeStore()
const data=ref(null),loading=ref(false),paying=ref(false),cancelling=ref(false),refunding=ref(false),refreshing=ref(false)
let timer
...
<template>
  <el-card v-loading="loading">
    <template #header><el-page-header @back="router.push('/user/orders')"><template #content>订单详情</template></el-page-header></template>
    <template v-if="data">
      ...
      <el-descriptions :column="2" border class="details">
        <el-descriptions-item label="订单号">{{order.orderNo}}</el-descriptions-item><el-descriptions-item label="状态"><StatusTag :value="order.orderStatus"/></el-descriptions-item>
        <el-descriptions-item label="比赛">{{order.homeClubName}} vs {{order.awayClubName}}</el-descriptions-item><el-descriptions-item label="比赛时间">{{$formatDateTime(order.matchTime)}}</el-descriptions-item>
        <el-descriptions-item label="场馆">{{order.stadiumName}}</el-descriptions-item><el-descriptions-item label="比赛票区">{{order.zoneName}}</el-descriptions-item>
        <el-descriptions-item label="数量">{{order.ticketCount}}张</el-descriptions-item><el-descriptions-item label="总金额">{{$formatMoney(order.totalAmount)}}</el-descriptions-item>
        <el-descriptions-item v-if="order.paidAt" label="支付时间">{{$formatDateTime(order.paidAt)}}</el-descriptions-item><el-descriptions-item v-if="order.cancelReason" label="取消原因">{{$statusLabel(order.cancelReason)}}</el-descriptions-item>
      </el-descriptions>
      ...
      <div v-if="data.tickets?.length"><h3>电子票</h3><el-button v-for="t in data.tickets" :key="t.ticketId" type="success" plain @click="router.push(`/user/tickets/${t.ticketId}`)">{{t.passengerName}} · {{t.rowNo}}排 {{t.seatNo}}座</el-button></div>
      <div v-if="order.orderStatus==='PENDING_PAYMENT'" class="pay-actions"><el-button type="primary" :loading="paying" :disabled="!remaining||cancelling" @click="pay('SUCCESS')">模拟支付成功</el-button><el-button :loading="paying" :disabled="!remaining||cancelling" @click="pay('FAILED')">模拟支付失败</el-button><el-button type="danger" plain :loading="cancelling" :disabled="paying" @click="cancel">取消订单</el-button></div>
      <el-button v-if="order.orderStatus==='PAID'" class="pay-actions" type="warning" :loading="refunding" @click="refund">申请整单退票</el-button>
    </template>
