...
<template>
  <div class="order-page">
    ...
    <DataState :loading="loading" :error="error" :empty="!data" empty-title="订单不存在或无权查看" @retry="load">
      <el-alert v-if="order.orderStatus === 'PENDING_PAYMENT'" type="warning" :closable="false"
        :title="remaining ? `支付剩余时间 ${countdown}` : '订单正在关闭，请稍候刷新'" />
      <el-alert v-if="order.orderStatus === 'REFUND_PENDING'" title="历史退款处理中，座位和电子票暂时保留" type="warning"
        :closable="false" />
      <section class="order-summary">
        <div class="order-summary__status"><span>订单状态</span>
          <h2>{{ $statusLabel(order.orderStatus) }}</h2><small>订单号 {{ order.orderNo }}</small>
        </div>
        <div class="order-summary__match"><span>比赛</span>
      ...
      <section class="order-content">
        <div class="order-content__main">
          ...
          <CardShell v-if="data.tickets?.length" class="ticket-section" title="电子票" subtitle="支付成功后可分别查看每位购票人的入场票券。">
            <div class="ticket-links">
              <RouterLink v-for="ticket in data.tickets" :key="ticket.ticketId" :to="`/user/tickets/${ticket.ticketId}`"
                class="ticket-link"><span>{{ ticket.passengerName }}</span><b>{{ ticket.rowNo }}
                  {{ ticket.seatNo }}</b><em>查看电子票</em></RouterLink>
            </div>
          </CardShell>
        </div>
        <aside class="order-meta">
          ...
          <dl>
            ...
            <div v-if="order.cancelReason">
              <dt>取消原因</dt>
              <dd>{{ $statusLabel(order.cancelReason) }}</dd>
            </div>
          </dl>
      ...
      <div v-if="data.refund || data.payment" class="transaction-grid">
        <CardShell v-if="data.refund" title="退款信息">
          <dl class="detail-list">
            <div>
              <dt>退款编号</dt>
              <dd>{{ data.refund.refundNo }}</dd>
            </div>
            <div>
              <dt>退款状态</dt>
              <dd>
                <StatusTag :value="data.refund.refundStatus" />
              </dd>
            </div>
            <div>
              <dt>退款金额</dt>
              <dd>{{ $formatMoney(data.refund.refundAmount) }}</dd>
            </div>
            <div>
              <dt>手续费</dt>
              <dd>{{ $formatMoney(data.refund.feeAmount) }}</dd>
            </div>
            <div>
              <dt>退款比例</dt>
              <dd>{{ Number(data.refund.refundRate || 0) * 100 }}%</dd>
            </div>
            <div>
              <dt>处理方式</dt>
              <dd>{{ data.refund.processingMode === 'AUTO' ? '系统自动' : '历史人工' }}</dd>
            </div>
            <div>
              <dt>申请时间</dt>
              <dd>{{ $formatDateTime(data.refund.appliedAt) }}</dd>
            </div>
            <div>
              <dt>退款原因</dt>
              <dd>{{ data.refund.reason }}</dd>
            </div>
            <div v-if="data.refund.auditReason">
              <dt>历史审核结果</dt>
              <dd>{{ data.refund.auditReason }}</dd>
            </div>
          </dl>
        </CardShell>
        <CardShell v-if="data.payment" title="演示支付记录" subtitle="仅用于系统演示，不代表真实支付渠道。">
          <dl class="detail-list">
            <div>
              <dt>支付流水</dt>
              <dd>{{ data.payment.paymentNo }}</dd>
            </div>
            <div>
              <dt>支付状态</dt>
              <dd>
                <StatusTag :value="data.payment.payStatus" />
              </dd>
            </div>
          </dl>
        </CardShell>
