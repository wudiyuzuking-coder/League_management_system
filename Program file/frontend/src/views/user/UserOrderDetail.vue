...
<template>
  <div class="order-page">
    ...
    <DataState :loading="loading" :error="error" :empty="!data" empty-title="订单不存在或无权查看" @retry="load">
      ...
      <div v-if="data.refund||data.payment" class="transaction-grid">
        <CardShell v-if="data.refund" title="退款信息"><dl class="detail-list"><div><dt>退款编号</dt><dd>{{data.refund.refundNo}}</dd></div><div><dt>退款状态</dt><dd><StatusTag :value="data.refund.refundStatus"/></dd></div><div><dt>退款金额</dt><dd>{{$formatMoney(data.refund.refundAmount)}}</dd></div><div><dt>手续费</dt><dd>{{$formatMoney(data.refund.feeAmount)}}</dd></div><div><dt>退款比例</dt><dd>{{Number(data.refund.refundRate||0)*100}}%</dd></div><div><dt>处理方式</dt><dd>{{data.refund.processingMode==='AUTO'?'系统自动':'历史人工'}}</dd></div><div><dt>申请时间</dt><dd>{{$formatDateTime(data.refund.appliedAt)}}</dd></div><div><dt>退款原因</dt><dd>{{data.refund.reason}}</dd></div><div v-if="data.refund.auditReason"><dt>历史审核结果</dt><dd>{{data.refund.auditReason}}</dd></div></dl></CardShell>
        <CardShell v-if="data.payment" title="演示支付记录" subtitle="仅用于系统演示，不代表真实支付渠道。"><dl class="detail-list"><div><dt>支付流水</dt><dd>{{data.payment.paymentNo}}</dd></div><div><dt>支付状态</dt><dd><StatusTag :value="data.payment.payStatus"/></dd></div></dl></CardShell>
      </div>
    </DataState>
  </div>
</template>
...
.order-content{display:grid;grid-template-columns:minmax(0,1fr) 300px;align-items:start;gap:var(--space-lg)}.order-content__main{display:grid;gap:var(--space-4)}.order-meta{border:1px solid var(--border-color);border-radius:var(--radius-lg);background:var(--surface)}.order-meta h2{margin:0;font-size:18px}
...
.ticket-links{display:grid;grid-template-columns:repeat(2,1fr);gap:var(--space-sm)}.ticket-link{display:grid;grid-template-columns:1fr auto;gap:5px;padding:14px;border:1px solid var(--border-color);border-radius:var(--radius-md)}.ticket-link em{grid-column:1/-1;color:var(--primary);font-size:12px;font-style:normal}.ticket-link:hover{border-color:var(--primary);background:var(--surface-muted)}.transaction-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:var(--space-4);margin-top:var(--space-lg)}.detail-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:var(--space-4);margin:0}.detail-list dt{color:var(--color-text-muted);font-size:var(--font-size-xs)}.detail-list dd{margin:var(--space-1) 0 0;font-weight:var(--font-weight-medium)}@media(max-width:900px){.order-summary{grid-template-columns:1fr}.order-summary__status{border-right:0}.order-summary__amount{align-items:flex-start}.order-content,.transaction-grid{grid-template-columns:1fr}}
</style>
