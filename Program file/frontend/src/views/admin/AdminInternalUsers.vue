...
const confirmVisible = ref(false), pendingUser = ref(null)
...
const requestToggle = row => { if (row.userStatus === 'PENDING_ACTIVATION') return ElMessage.warning('等待本人完成首次启用'); if (row.userStatus === 'CANCELLED') return; pendingUser.value = row; confirmVisible.value = true }
const confirmToggle = async () => { const row = pendingUser.value; if (!row || actionUserId.value) return; const enabling = row.userStatus !== 'ENABLED'; actionUserId.value = row.userId; try { await updateInternalUserStatus(row.userId, enabling ? 'ENABLED' : 'DISABLED'); ElMessage.success(enabling ? '账号已重新启用' : '账号已停用'); confirmVisible.value = false; await load() } catch (e) { if (!e?.__notified) ElMessage.error(e?.message || '账号状态更新失败') } finally { actionUserId.value = null } }
onMounted(load)
</script>

<template>
  <div class="governance-page">
    <PageHeader :breadcrumb="[{label:'系统管理'},{label:'内部人员'}]" title="内部人员管理" subtitle="预登记赛事管理员和系统管理员，并管理账号状态。"><template #actions><el-button type="primary" @click="open">新增管理账号</el-button></template></PageHeader>
    <el-alert class="page-alert" type="info" :closable="false" title="预登记后状态为待激活；本人使用手机号和 4 位工号数字设置密码。" />
    <FilterBar :model="query">
      <el-form-item label="姓名"><el-input v-model="query.username" name="internal-user-name" autocomplete="off" clearable placeholder="输入姓名…" /></el-form-item>
      ...
      <el-form-item label="状态"><el-select v-model="query.userStatus" clearable placeholder="全部状态"><el-option v-for="option in statusOptions" :key="option.value" :label="option.label" :value="option.value" /></el-select></el-form-item>
      <template #actions><el-button @click="reset">重置</el-button><el-button type="primary" @click="query.page=1;load()">查询</el-button></template>
    </FilterBar>
    <TableWrapper label="内部人员列表">
      <DataState :loading="loading" :error="error" :empty="!rows.length" empty-title="暂无内部人员" empty-description="可以预登记新的管理账号，或调整筛选条件。" @retry="load">
        <el-table :data="rows"><el-table-column prop="realName" label="姓名" min-width="120" show-overflow-tooltip /><el-table-column label="手机号" min-width="130"><template #default="{row}">{{ maskPhone(row.phone) }}</template></el-table-column><el-table-column prop="employeeNo" label="工号" min-width="110" /><el-table-column label="类型" min-width="130"><template #default="{row}">{{ roleName(row.roleCode) }}</template></el-table-column><el-table-column label="状态" width="110"><template #default="{row}"><StatusTag :value="row.userStatus" /></template></el-table-column><el-table-column label="操作" width="144" fixed="right"><template #default="{row}"><span v-if="row.userStatus === 'CANCELLED'" class="muted-text">不可操作</span><el-button v-else link :disabled="row.userStatus === 'PENDING_ACTIVATION'" :type="row.userStatus === 'ENABLED' ? 'danger' : 'primary'" @click="requestToggle(row)">{{ row.userStatus === 'ENABLED' ? '停用' : '启用' }}</el-button></template></el-table-column></el-table>
      </DataState>
    ...
    <el-dialog v-model="visible" class="app-dialog" title="预登记管理账号" width="520px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="110px"><el-form-item label="管理员类型" prop="roleCode"><el-select v-model="form.roleCode"><el-option v-for="option in roleOptions" :key="option.value" :label="option.label" :value="option.value" /></el-select></el-form-item><el-form-item label="手机号" prop="phone"><el-input v-model="form.phone" name="management-phone" type="tel" inputmode="tel" autocomplete="off" placeholder="例如：13800000000…" /></el-form-item><el-form-item label="姓名" prop="realName"><el-input v-model="form.realName" name="management-real-name" autocomplete="off" placeholder="输入真实姓名…" /></el-form-item><el-form-item label="4位工号数字" prop="employeeNo"><el-input v-model="form.employeeNo" name="management-employee-no" inputmode="numeric" autocomplete="off" maxlength="4" placeholder="例如：0001…"><template #prepend>{{ prefix }}</template></el-input></el-form-item></el-form>
      <template #footer><el-button :disabled="saving" @click="visible=false">取消</el-button><el-button type="primary" :loading="saving" @click="save">确认预登记</el-button></template>
    </el-dialog>
    <ConfirmDialog v-model="confirmVisible" :title="pendingUser?.userStatus === 'ENABLED' ? '停用管理账号' : '启用管理账号'" :message="`将${pendingUser?.userStatus === 'ENABLED' ? '停用' : '启用'}“${pendingUser?.realName || ''}”的管理账号。`" :impact="pendingUser?.userStatus === 'ENABLED' ? '停用后该人员将无法登录；历史操作和业务数据会保留。' : '启用后该人员可以重新登录；历史操作和业务数据保持不变。'" :confirm-text="pendingUser?.userStatus === 'ENABLED' ? '确认停用' : '确认启用'" :danger="pendingUser?.userStatus === 'ENABLED'" :loading="actionUserId !== null" @confirm="confirmToggle" />
  </div>
</template>

<style scoped>.page-alert{margin-bottom:var(--space-4)}.muted-text{color:var(--color-text-muted);font-size:var(--font-size-sm)}</style>
