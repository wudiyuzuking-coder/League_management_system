...
- `CLUB`：维护俱乐部、私有标准主场、人员阵容、赛季报名、已发布赛程和俱乐部数据。
- `EVENT_ADMIN`：创建赛季、管理报名、查看系统自动发布的赛程与比赛、独立提交赛果并查看赛季营收。
...
- 报名窗口和预计结束日期由系统基于 `SystemTime` 与正式排赛规则生成；历史 `season.ticket_sale_start_time` 不参与单场售票判定。
- 正式赛程为主客场双循环，每天最多一场，同一 CLUB 相邻比赛至少间隔 6 天，统一 20:00。
- 每场比赛在比赛日期前 14 天的 20:00 自动开售，停售时间固定为比赛开始前 1 小时。赛程展示、查询、Preview 和 Order 共用唯一 `TicketSalePolicy`。
